...
def populate_cast(table,ids):
    ...
    castattrs=['id','name','gender','credit_id','order','profile_path']
    personattrs=['adult','also_known_as','biography','birthday','deathday','homepage','imdb_id','birthplace','popularity']
    for id in ids:
        sleep(0.26)
        ...
        if(hasattr(credits, 'cast')):
            for m in credits.cast[:10]:
                ...
                order=m.get('order') if m.get('order') != '' else None
